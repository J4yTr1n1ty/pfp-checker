...
use serenity::all::{
    CommandInteraction, Context, CreateCommandOption, CreateEmbed, CreateEmbedAuthor, CreateEmbedFooter, CreateInteractionResponse, CreateInteractionResponseMessage, ResolvedValue
};
                ...
                match pfps {
                   Ok(entries) => {
                        ...
                        for entry in &entries {
                            ...
                            let dt = DateTime::<Utc>::from_timestamp(entry.changedAt.unwrap(), 0).expect("Invalid timestamp");
                            ...
                            if count > 0 {
                                ...
                                let prev_dt = DateTime::<Utc>::from_timestamp(prev_entry.changedAt.unwrap(), 0).expect("Invalid timestamp");
                        ...
                        if count > 1 { // Ensure there's more than one entry to calculate an average
